        ...
        if resp.status_code == 200:
            ...
            for row in rows:
                ...
                if len(cols) >= 3:
                    ...
                    if ip.count('.') == 3:
                        if '电信' in isp: ips['CT'].append(ip)
                        elif '联通' in isp: ips['CU'].append(ip)
                        elif '移动' in isp: ips['CM'].append(ip)
    except Exception as e:
        print(f"Error fetching from uouin: {e}")
    return ips
    ...
    try:
        ...
        if resp.status_code == 200:
            for item in resp.json():
                ip, line = item.get('ip'), item.get('line', '')
                if ip:
...
def main():
    ...
    for isp_code, isp_name in [('CT', '电信'), ('CU', '联通'), ('CM', '移动')]:
        ...
        print(f"✅ 【{isp_name}】最佳 2 个 IP: {top_2}")
    ...
    print("🎉 订阅文件 sub.yaml 生成成功！所有 xxx.xxx.xxx.xxx 已全部更新为真实优选 IP！")
